...
__all__ = ["webpage_extractor", "WebPage", "Section", "embed_webpage", "async_embed_webpage", "OpenAIEmbedder", "AsyncOpenAIEmbedder", "Embedder", "AsyncEmbedder"]
...
class OpenAIEmbedder:
    def __init__(self, model: Literal["text-embedding-3-small", "text-embedding-3-large"], section_chunker=None):
        self.model = model
        self.client = OpenAI()
    ...
    def __call__(self, section: Section) -> list[list[float]]:
        ...
        embeddings = [openai.embeddings.create(
            input=subsection, model=self.model
        ).data[0].embedding for subsection in subsections]
        return embeddings
...
async def _async_embed_section(section: Section, embedder: AsyncEmbedder) -> list[float]:
    return await embedder(section)

def embed_webpage(webpage: WebPage, embedder: Embedder) -> list[tuple[Section, list[list[float]]]]:

    return [(section, embedder(section)) for section in webpage.sections]

async def async_embed_webpage(webpage: WebPage, embedder: AsyncEmbedder) -> list[tuple[Section, list[list[float]]]]:
    result = []
...
def compute_distance(section_embedding: list[list[float]], query_embedding: list[float], power=1) -> float:
    
    return (sum(_cosine_similarity(emb, query_embedding)**power for emb in section_embedding) / len(section_embedding))**(1,0/power)
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        while True:
            ...
            embeded_webpage = await async_embed_webpage(results, AsyncOpenAIEmbedder("text-embedding-3-small", section_chunker=SentenceSplitter(chunk_size=2048, chunk_overlap=40)))
            while True:
                ...
                console.print(Markdown(f"# Average Embedding Distance"))
                sorted_sections = sorted(embeded_webpage, key=lambda x: sum(_cosine_similarity(emb, query_embedding) for emb in x[1])/len(x[1]), reverse=True)
                for section, embedding in sorted_sections[:3]:
                    console.print(Markdown(f"# Distance: {sum(_cosine_similarity(emb, query_embedding) for emb in embedding)/len(embedding): .4f}\n\n## Length: {len(embedding)}"))
                    console.print(Markdown(section.markdown))
                    console.print(Markdown("---"))
                
                console.print(Markdown(f"# Distance from Average Embedding"))
                sorted_sections = sorted(embeded_webpage, key=lambda x: _cosine_similarity(np.mean(x[1], axis=0), query_embedding), reverse=True)
                for section, embedding in sorted_sections[:3]:
                    console.print(Markdown(f"# Distance: {_cosine_similarity(np.mean(embedding, axis=0), query_embedding): .4f}\n\n## Length: {len(embedding)}"))
                    console.print(Markdown(section.markdown))
                    console.print(Markdown("---"))
    asyncio.run(main())
